...
class FullMatrixRunPlanTests(unittest.TestCase):
    def test_run_plan_contains_three_seed_six_fold_seven_model_matrix(self):
        from training.full_matrix_runner import build_full_matrix_run_plan, load_full_matrix_config
        ...
        plan = build_full_matrix_run_plan(config, splits)

        self.assertEqual(len(plan), 126)
        self.assertEqual(len({row["run_id"] for row in plan}), 126)
        self.assertEqual(plan[0]["status"], "pending")
        self.assertEqual(plan[0]["output_subdir"], "all_channel_conv1d_v1/seed_42/fold_01")
