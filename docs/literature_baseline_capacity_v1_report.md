...
| feature_random_forest_v1 |  |  |  |  | classical baseline; CAU sklearn_available=True, local availability may vary |
| feature_linear_svm_v1 |  |  |  |  | classical baseline; CAU sklearn_available=True, local availability may vary |
...
- CAU screening 환경에서는 scikit-learn이 설치되어 있어 `feature_random_forest_v1`과 `feature_linear_svm_v1`이 모두 실행됐다.
