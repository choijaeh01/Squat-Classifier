...
from analysis.professor_report_v3_assets import (  # noqa: E402
    ProfessorReportV3Paths,
    build_professor_report_v3_assets,
    check_professor_report_v3_inputs,
)
...
def main() -> int:
    ...
    is_v3 = output_docs.name == "professor_report_v3"
    if is_v3:
        default_mapping = PROJECT_ROOT / "docs" / "professor_report_v3" / "display_name_mapping.yaml"
    else:
    ...
    if is_v3:
        status = check_professor_report_v3_inputs(project_root)
    else:
    ...
    if is_v3:
        paths_v3 = ProfessorReportV3Paths(
            project_root=project_root,
            output_docs=output_docs,
            output_obsidian=args.output_obsidian,
            mapping_path=mapping_path,
        )
        result_v3 = build_professor_report_v3_assets(paths_v3)
        print(json.dumps(result_v3, ensure_ascii=False, indent=2))
        return 0 if result_v3["validation"]["passed"] else 1
